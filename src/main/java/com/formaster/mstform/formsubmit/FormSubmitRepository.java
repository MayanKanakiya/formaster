...
public interface FormSubmitRepository extends JpaRepository<FormSubmitEntity, Integer> {
	...
	@Query(value = "select sf.submitedon,f.fid,f.titletxt,u.fname,u.lname from mst_createform_entity as f join form_submit_entity as sf on sf.fid=f.fid join user_entity as u on sf.submitedby = u.id where sf.submitedby=?1 and sf.issubmited=1 and f.isdelete=0", nativeQuery = true)
